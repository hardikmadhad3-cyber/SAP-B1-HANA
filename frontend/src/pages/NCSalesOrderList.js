...
const STATUS_OPTIONS = [
  { code: 'Open', name: 'Open NC sales orders' },
  { code: 'Closed', name: 'Closed NC sales orders' },
];
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        if (!ignore) {
          setPageState({
            loading: false,
            error: getErrorMessage(error, 'Failed to load NC sales orders.'),
          });
        }
      }
    };
                  ...
                  <tr key={order.doc_entry}>
                    <td>
                      <button
                        ...
                        onClick={() =>
                          navigate('/nc-sales-order', {
                            state: createCompanyScopedRouteState({
                              ncSalesOrderDocEntry: order.doc_entry,
...
export default NCSalesOrderListPage;
