...
const STATUS_OPTIONS = [
  { code: 'Open', name: 'Open DC sales orders' },
  { code: 'Closed', name: 'Closed DC sales orders' },
];
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        if (!ignore) {
          setPageState({
            loading: false,
            error: getErrorMessage(error, 'Failed to load DC sales orders.'),
          });
        }
      }
    };
              ...
              {!pageState.loading && orders.length === 0 && (
                <tr>
                  <td colSpan="12" className="text-center text-muted py-4">
                    {hasActiveFilters ? 'No DC sales orders found for the selected filters.' : 'No DC sales orders found.'}
                  </td>
                </tr>
              )}
                  ...
                  <tr key={order.doc_entry}>
                    <td>
                      <button
                        ...
                        onClick={() =>
                          navigate('/dc-sales-order', {
                            state: createCompanyScopedRouteState({
                              dcSalesOrderDocEntry: order.doc_entry,
...
export default DCSalesOrderListPage;
