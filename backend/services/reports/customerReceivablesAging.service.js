...
const getLookups = async () => {
  const [groups, accounts, employees, properties, currencyRows] = await Promise.all([
    queryRows(`
      ...
      WHERE BP.CardType = 'C' ORDER BY name
    `),
    queryRows(`
      ...
      WHERE BP.CardType = 'C' ORDER BY A.AcctCode
    `),
  ...
  return {
    customerGroups: [{ code: "All", name: "All" }, ...groups],
...
const getReport = async (criteria = {}) => {
  ...
  const clauses = ["BP.CardType = 'C'", "CAST(L.RefDate AS DATE) <= CAST(@agingDate AS DATE)"];
  ...
  if (criteria.ignoreFutureRemit) {
    clauses.push("NOT (L.TransType = 24 AND CAST(L.RefDate AS DATE) > CAST(@agingDate AS DATE))");
  }
  ...
  const rows = await queryRows(`
    SELECT TOP 5000
      ...
      ISNULL(S.SlpCode, -1) AS SlpCode, ISNULL(S.SlpName, 'No Sales Employee') AS SlpName,
  ...
  const resultRows = rows.map((row, index) => {
    ...
    const daysOutstanding = Math.max(0, Math.floor((new Date(`${agingDate}T00:00:00`) - new Date(ageDate)) / 86400000));
    ...
    buckets[bucketIndex(daysOutstanding, intervals)] = balance;
    ...
    return {
      ...
      salesEmployeeCode: String(row.SlpCode ?? ""),
      salesEmployeeName: row.SlpName || "No Sales Employee",
  ...
  }).filter((row) => criteria.displayZeroBalance || criteria.displayReconciled || Math.abs(row.balance) > 0.000001);

  return {
    reportTitle: "Customer Receivables Aging",
    generatedAt: new Date().toISOString(),
    agingDate,
    ageBy: criteria.ageBy || "due",
    displayCurrency: criteria.displayCurrency || "local",
    groupBy: criteria.groupBy || "customer",
    intervals,
    rows: resultRows,
  };
};
