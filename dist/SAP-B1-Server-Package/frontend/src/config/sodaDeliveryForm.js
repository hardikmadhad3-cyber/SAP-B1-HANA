const FORM_SETTINGS_STORAGE_KEY = 'sapb1.sodaDelivery.formSettings.v4';
...
const SODA_DELIVERY_MATRIX_COLUMN_UDF_KEYS = {
...
const SODA_DELIVERY_MATRIX_COLUMNS = [
  ...
  { key: 'commissionPercent', label: 'Commission (Percent)', visible: true },
  { key: 'commissionAmount', label: 'Commission', visible: true },
  ...
  { key: 'commissionAmountPerTon', label: 'comm amt per tone', visible: true },
  { key: 'hsnCode', label: 'HSN', visible: true },
  { key: 'sacCode', label: 'SAC', visible: true },
  { key: 'commissionBy', label: 'comissin by', visible: true },
  ...
  { key: 'deliveredQty', label: 'Qty to Ship', visible: true },
...
const BASE_MATRIX_COLUMNS = SODA_DELIVERY_MATRIX_COLUMNS.map((field) => ({
  ...field,
...
const buildSapUdfKeySet = (definitions = []) =>
  (definitions || []).reduce((keys, field = {}) => {
    [field.key, field.sapField, field.aliasId].forEach((value) => {
      const normalized = normalizeUdfKey(value);
...
const buildCompanyMatrixColumnSettings = (rowUdfDefinitions = [], savedSettings = {}) => {
  ...
  return BASE_MATRIX_COLUMNS.reduce((acc, field) => {
    const requiredUdfKey = SODA_DELIVERY_MATRIX_COLUMN_UDF_KEYS[field.key];
    const hasRequiredUdf = !requiredUdfKey || availableUdfKeys.has(normalizeUdfKey(requiredUdfKey));
