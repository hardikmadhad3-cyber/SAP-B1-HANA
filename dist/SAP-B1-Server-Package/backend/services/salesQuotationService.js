  ...
  try {
    ...
    const sapPayload = {
      ...
      DocumentLines: payload.lines.map(l => {
        ...
        Object.entries(l.udf || {}).forEach(([key, value]) => {
          if (String(key || '').startsWith('U_') && value !== undefined && value !== null && String(value).trim() !== '') {
            line[key] = value;
          }
        });
  ...
  try {
    ...
    const sapPayload = {
      ...
      DocumentLines: payload.lines.map(l => ({
        ...
        WarehouseCode: l.whse || '01',
        TaxCode: l.taxCode || 'IGST5',
        ...Object.entries(l.udf || {}).reduce((acc, [key, value]) => {
          if (String(key || '').startsWith('U_') && value !== undefined && value !== null && String(value).trim() !== '') {
            acc[key] = value;
          }
          return acc;
